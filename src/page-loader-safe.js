...
function showFatal(error) {
  console.error('[Sprite Sheet Studio] safe startup failed', error);
  if (!app) return;
  ...
  app.innerHTML = `
    <main style="min-height:100vh;display:grid;place-items:center;background:#080d16;color:#e8edf6;font:14px system-ui;padding:24px">
      <div style="max-width:680px;border:1px solid #213149;border-radius:14px;background:#0e1624;padding:24px">
        <img src="./assets/brand/logo-mark.svg" alt="" width="56" height="56" />
        <h1 style="margin:12px 0 8px;font-size:20px">Sprite Sheet Studio</h1>
        <p style="margin:0 0 12px;color:#9cadc3;line-height:1.55">Safe startup failed before the editor initialized.</p>
...
function stripMainTypeScript(source) {
  ...
  code = code
    ...
    .replaceAll('./logo-mark.svg', './assets/brand/logo-mark.svg');

  return stripFunctionTypes(code);
}
...
async function boot() {
  document.documentElement.dataset.runtime = 'safe-core';
  const source = await fetchText(new URL('./main-v2.ts', import.meta.url), 'core editor source');
  const js = `${stripMainTypeScript(source)}\n//# sourceURL=sprite-sheet-studio-safe-runtime.js`;
  const blobUrl = URL.createObjectURL(new Blob([js], { type: 'text/javascript' }));
  try {
    await import(blobUrl);
    document.documentElement.dataset.runtimeReady = '1';
    console.info('[Sprite Sheet Studio] safe core started');
  } finally {
