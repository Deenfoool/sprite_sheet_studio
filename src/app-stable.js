...
const el = {
  ...
  frameCount: $('#frameCount'), canvasSize: $('#canvasSize'), duration: $('#durationValue'), timelineInfo: $('#timelineInfo'), previewLabel: $('#previewLabel'), toast: $('#toast')
};
...
async function loadFiles(files) {
  ...
  state.playing = false;
  if (list.length === 1) {
    const canvas = await fileToCanvas(list[0]); state.source = canvas; state.sourceName = list[0].name; el.rows.value = '1'; el.cols.value = '1'; sliceSource();
  } else {
    const frames = []; for (const file of list.sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true }))) frames.push({ id: uid(), name: file.name, canvas: await fileToCanvas(file) });
...
function duplicate() { const f = current(); if (!f) return; state.frames.splice(state.index + 1, 0, { id: uid(), name: f.name.replace(/\.png$/i, '-copy.png'), canvas: cloneCanvas(f.canvas) }); state.index++; renderAll(); }
...
function exportSheet() {
  ...
  const out = document.createElement('canvas'); out.width = w * state.frames.length; out.height = h; const ctx = ctx2d(out); ctx.imageSmoothingEnabled = false;
...
el.fileInput.addEventListener('change', () => loadFiles(el.fileInput.files).catch((e) => { console.error(e); toast('Could not load image'); }));
...
el.uploadZone.addEventListener('drop', (e) => loadFiles(e.dataTransfer.files).catch(console.error));
